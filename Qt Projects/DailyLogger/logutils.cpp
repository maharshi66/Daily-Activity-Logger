...
namespace LOGUTILS
{
  ...
  {
    logFileName = QString(logFolderName + "/_%1.txt").arg(QDate::currentDate().toString("yyyy_MM_dd"));
  }
  ...
  void myMessageHandler(QtMsgType type, const QMessageLogContext &context, const QString& txt)
  {
    ...
    ts << txt << "\t" << QDate::currentDate().toString("dd/MM/yyyy") <<"\t"<<QTime::currentTime().toString("hh:mm:ss")<<  endl;
  }
}
